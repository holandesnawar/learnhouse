...
## Hecho
- **Barra lateral izquierda** (`apps/web/components/Objects/Menus/OrgSidebar.tsx`, usada en `apps/web/app/orgs/[orgslug]/(withmenu)/layout.tsx`): azul Nawar profundo + glow blanco + puntos, texto blanco, activo #4da3ff, **botón colapsar** (persistido en localStorage), cajón móvil. Reemplaza el menú superior `OrgMenu`. (Vive en la rama `claude/adoring-dijkstra-rI3FL`, aún no fusionada a `dev`.)
