...
- **Campana de notificaciones del alumno** (`NotificationsBell.tsx`, en la cabecera de `OrgSidebar` y en la barra superior de móvil): avisa cuando le mencionan en la comunidad (`@nombre` o `@all`). Tabla nueva `notification_seen` (solo `user_id` + `last_seen_at`, sin migración). Endpoints `GET /community-engagement/notifications` y `PUT /community-engagement/notifications/seen`. Abrir la campana apaga el punto rojo pero NO marca los canales como leídos.
