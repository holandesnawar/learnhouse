...
const CourseActionsMobile = ({ courseuuid, orgslug, course, trailData }: CourseActionsMobileProps) => {
  ...
  if (session.data?.user && !isUserPartOfTheOrg) {
    return (
      <div className="bg-white/90 backdrop-blur-sm shadow-md shadow-gray-300/25 outline outline-1 outline-neutral-200/40 rounded-2xl overflow-hidden p-4 my-6">
      ...
      <div className="flex flex-col space-y-4">
        {hayOferta ? (() => {
          ...
          <button
            ...
            className="w-full py-2 px-4 rounded-lg font-semibold text-sm transition-colors flex items-center justify-center gap-2 bg-[#4da3ff] text-[#1D0084] hover:bg-[#6cb5ff] disabled:opacity-70"
          >
            {isActionLoading ? (
              <div className="w-4 h-4 border-2 border-[#1D0084] border-t-transparent rounded-full animate-spin" />
            ) : !session.data?.user ? (
              <>
