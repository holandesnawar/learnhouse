...
// Keys come from the runtime config (getConfig), since NEXT_PUBLIC_* vars are
// injected at runtime in this deployment, not inlined at build time.
const supabaseUrl = getConfig('NEXT_PUBLIC_SUPABASE_URL', '');
const supabaseAnonKey = getConfig('NEXT_PUBLIC_SUPABASE_ANON_KEY', '');
